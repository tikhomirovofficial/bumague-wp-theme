<?php
/**
 * Секция портфолио с AJAX-фильтрацией (новая версия)
 */

// Получаем все записи портфолио (категории)
$portfolio_posts = get_posts([
    'post_type' => 'gallery_category', // Тип записи "Галерея"
    'posts_per_page' => -1,
    'orderby' => 'date',
    'order' => 'DESC',
    'post_status' => 'publish'
...
// Получаем первые изображения для первоначальной загрузки
$initial_images = [];
...
// Сортируем все изображения по дате (новые сначала)
usort($initial_images, function ($a, $b) {
    return strtotime($b['post_date']) - strtotime($a['post_date']);
});

// Берем только первые 12 для начальной загрузки
$initial_images = array_slice($initial_images, 0, 12);
?>
            ...
            <div class="portfolio__inner">
                ...
                <ul class="portfolio__tabs">
                    ...
                    <?php foreach ($portfolio_posts as $post): ?>
                        <li class="portfolio__tabs-item">
                            <button class="filter-item" data-category="<?php echo esc_attr($post->ID); ?>">
                ...
                <div class="portfolio__gallery">
                        ...
                        <?php if (!empty($initial_images)): ?>
                            <?php foreach ($initial_images as $image): ?>
                                <li class="portfolio__gallery-item"
                                    ...
                                    data-post="<?php echo esc_attr($image['post_id']); ?>">
                                </li>
                            <?php endforeach; ?>
                        <?php else: ?>
                    ...
                    <!-- Кнопка загрузки -->
                    <?php if (count($initial_images) == 12): ?>
                        <button class="portfolio__gallery-load" id="load-more-portfolio" data-page="1" data-category="all">
                            <span>Загрузить еще</span>
                            <img src="<?php echo esc_url(get_template_directory_uri() . '/images/icons/arro-down-right-dark.svg'); ?>"
                                alt="">
                        </button>
                    <?php endif; ?>
                </div>
    ...
    jQuery(document).ready(function ($) {
        ...
        $('.portfolio__tabs .filter-item').on('click', function () {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'filter_portfolio_gallery',
                    category: category,
                    page: 1,
                ...
                success: function (response) {
                    ...
                    // Проверяем наличие маркера окончания загрузки
                    var hasMoreItems = $(response).filter('.no-more-items').length === 0;
                    $('#load-more-portfolio').toggle(hasMoreItems);
                },
                error: function () {
        ...
        $('#load-more-portfolio').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if ($(response).filter('.portfolio__gallery-item').length > 0) {
                        $('#portfolio-gallery').append(response);
                        button.data('page', page).find('span').text('Загрузить еще').prop('disabled', false);

                        // Скрываем кнопку, если есть маркер окончания
                        if ($(response).filter('.no-more-items').length > 0) {
                            button.hide();
                        }
                    } else {
                        // Если нет элементов - скрываем кнопку
                        button.hide();
                    }
                },
                error: function () {
